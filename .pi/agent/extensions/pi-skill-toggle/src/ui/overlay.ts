...
class SkillToggleOverlay {
  ...
  handleInput(data: string): void {
    if (matchesKey(data, Key.escape)) {
      // Esc in search mode just returns to normal mode; otherwise cancels the dialog.
    ...
    if (!this.searchFocused) {
      this.handleNormalModeInput(data);
      return;
    }

    this.handleSearchModeInput(data);
  }

  private handleNormalModeInput(data: string): void {
  ...
  private handleSearchModeInput(data: string): void {
    // Enter also leaves search mode and returns to normal mode.
  ...
  render(width: number): string[] {
    ...
    const searchLabel = this.searchFocused ? "Search*" : "Search";
    const searchText = this.searchFocused ? `${this.search}█` : this.search || "(press s to search)";
    const search = frameLine(this.theme, this.theme.fg(this.searchFocused ? "accent" : "muted", `${searchLabel}: ${searchText}`), innerWidth);
    ...
    const footer = [
      frameLine(this.theme, this.theme.fg("dim", "j/k move • space toggle • s search • esc/enter leave search • ctrl+s apply + reload"), innerWidth),
